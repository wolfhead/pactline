...
function SettlementPanel() {
  ...
  async function run() {
    ...
    setRunning(true)
    try {
      const resp = await apiPost<SettlementResponse>('/api/settlements', {
...
function AnchorListPanel() {
  ...
  const load = useCallback(() => {
    apiGet<AnchorExample[]>('/api/anchors')
      .then(setAnchors)
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      await apiPost('/api/anchors', { dimension, level, bounty_id: bountyId, note })
      setBountyId('')
      setNote('')
      load()
    } catch (err) {
  ...
  async function remove(id: string) {
    setError('')
    try {
      await apiDelete(`/api/anchors/${id}`)
      load()
    } catch (err) {
      setError(String((err as Error).message))
    }
