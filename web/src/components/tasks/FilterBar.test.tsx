...
describe('FilterBar', () => {
  it('combines two status choices instead of replacing the first', async () => {
    const onChange = vi.fn()
    render(<FilterBar filters={{ ...DEFAULT_FILTERS, statuses: ['todo'] }} onChange={onChange}
      labels={[]} onLabelsChanged={() => {}} onRequestCreate={() => {}} />)
    fireEvent.click(screen.getByRole('button', { name: /状态/ }))
    fireEvent.click(await screen.findByRole('checkbox', { name: '进行中' }))
    // Decoy: an implementation that sets statuses to just the clicked one.
    expect(onChange).toHaveBeenCalledWith(expect.objectContaining({ statuses: ['todo', 'in_progress'] }))
  })

  it('asks the page to focus the capture row rather than opening a dialog', () => {
    const onRequestCreate = vi.fn()
    render(<FilterBar filters={DEFAULT_FILTERS} onChange={() => {}} labels={[]}
      onLabelsChanged={() => {}} onRequestCreate={onRequestCreate} />)
  ...
  it('marks a filter chip as active only when it actually narrows', () => {
    ...
    rerender(<FilterBar filters={{ ...DEFAULT_FILTERS, statuses: ['todo'] }} onChange={() => {}}
      labels={[]} onLabelsChanged={() => {}} onRequestCreate={() => {}} />)
  ...
  it('combines filters of different kinds into one request, and removing one leaves the others intact', async () => {
    ...
    const { rerender } = render(<FilterBar filters={DEFAULT_FILTERS} onChange={onChange} labels={[]}
      onLabelsChanged={() => {}} onRequestCreate={() => {}} />)

    fireEvent.click(screen.getByRole('button', { name: /状态/ }))
    fireEvent.click(await screen.findByRole('checkbox', { name: '进行中' }))
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ statuses: ['in_progress'], priorities: [] }))

    // The page would have committed that change and re-rendered with it —
    // simulate that before adding the second filter kind.
    rerender(<FilterBar filters={{ ...DEFAULT_FILTERS, statuses: ['in_progress'] }} onChange={onChange}
      labels={[]} onLabelsChanged={() => {}} onRequestCreate={() => {}} />)
    fireEvent.click(screen.getByRole('button', { name: /优先级/ }))
    fireEvent.click(await screen.findByRole('checkbox', { name: '高' }))
    expect(onChange).toHaveBeenLastCalledWith(
      expect.objectContaining({ statuses: ['in_progress'], priorities: ['high'] }),
    )

    // Now remove the status filter — the priority filter set alongside it
    // must survive untouched.
    rerender(<FilterBar filters={{ ...DEFAULT_FILTERS, statuses: ['in_progress'], priorities: ['high'] }}
      onChange={onChange} labels={[]} onLabelsChanged={() => {}} onRequestCreate={() => {}} />)
    fireEvent.click(screen.getByRole('button', { name: /状态/ }))
    fireEvent.click(await screen.findByRole('checkbox', { name: '进行中' }))
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ statuses: [], priorities: ['high'] }))
  })
})
