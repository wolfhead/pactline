...
// C1: mirrors `allowedTransitions` in internal/legacy/domain/bounty.go. The two must
...
export default function BountyDetail() {
  ...
  useEffect(() => {
    ...
    setError('')
    let cancelled = false
    apiGet<Bounty>(`/api/legacy/bounties/${id}`)
      .then((b) => {
        if (cancelled) return
        setBounty(b)
  ...
  async function move(to: Status) {
    ...
    setPending(to)
    try {
      await apiPost(`/api/legacy/bounties/${id}/transition`, {
        to,
  ...
  async function saveValueLevel() {
    ...
    try {
      await apiPost(`/api/legacy/bounties/${bounty.id}/value-level`, { value_level: valueLevelInput })
      setValueLevelInput('')
      reload()
    } catch (err) {
  ...
  async function saveDifficulty() {
    ...
    try {
      await apiPost(`/api/legacy/bounties/${bounty.id}/difficulty`, { difficulty: difficultyInput })
      setDifficultyInput('')
      reload()
    } catch (err) {
  ...
  // Mirrors domain.CanSetValueLevel in internal/legacy/domain/bounty.go: the
