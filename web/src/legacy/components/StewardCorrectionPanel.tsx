...
/**
 * The steward-only correction channel (POST /api/legacy/bounties/{id}/amend,
 * bountyHandler.amend in internal/legacy/api/bounty_handler.go). It is the only
...
export default function StewardCorrectionPanel({ bounty, onChanged }: { bounty: Bounty; onChanged: () => void }) {
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      await apiPost(`/api/legacy/bounties/${bounty.id}/amend`, {
        retrospective: retro,
