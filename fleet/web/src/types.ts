...
export interface VerificationMismatchDetail { category: string; command?: string; harness?: { outcome: string; summary: string }; fleet?: { outcome: string; exitCode: number | null; summary: string }; harnessChangedPaths?: string[]; fleetChangedPaths?: string[]; harnessChangedPathsOmitted?: number; fleetChangedPathsOmitted?: number }
export interface VerificationMismatch { at: string; stage: RunStage; role: 'implementer' | 'reviewer'; details: VerificationMismatchDetail[]; detailsOmitted?: number }
export interface RunDetail extends RunSummary { serviceId: string; configRevision: string; taskVersion?: number; claimId?: string; claimVersion?: number; claimTaskVersion?: number; runtimeSessionId?: string; workspace?: Record<string, string>; verificationMismatch?: VerificationMismatch; timeline: TimelineItem[]; effects: Effect[] }
