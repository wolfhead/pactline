...
//   - POST /api/bounties requires SPONSOR or STEWARD, so both bounties below
...
func TestPhase1FullLoop(t *testing.T) {
	...
	planRec := do(t, h, http.MethodPost, "/api/bounties", pmID, map[string]any{
	...
	delRec := do(t, h, http.MethodPost, "/api/bounties", pmID, map[string]any{
	...
	require.Equal(t, http.StatusOK, do(t, h, http.MethodPost,
		"/api/credits/"+inherited[0].ID.String()+"/respond", engCID,
	...
	supRec := do(t, h, http.MethodPost, "/api/bounties/"+delivery.ID.String()+"/credits", engDID,
	...
	require.Equal(t, http.StatusCreated, do(t, h, http.MethodPost,
		"/api/bounties/"+delivery.ID.String()+"/credits", engDID,
	...
	var deliveryView, abandonedView *api.WorkView
	for i := range feed {
	...
	portRec := do(t, h, http.MethodGet, "/api/users/"+engCID+"/portfolio", pmID, nil)
...
func listCredits(t *testing.T, h http.Handler, bountyID string) []domain.Credit {
	t.Helper()
	rec := do(t, h, http.MethodGet, "/api/bounties/"+bountyID+"/credits", pmID, nil)
