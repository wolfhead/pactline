...
func (h *creditHandler) nominate(w http.ResponseWriter, r *http.Request) {
	...
	if req.UserID == uuid.Nil {
		sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "user_id is required"})
		return
	}
...
func (h *creditHandler) respond(w http.ResponseWriter, r *http.Request) {
	...
	if req.Status != domain.CreditConfirmed && req.Status != domain.CreditDeclined {
		sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "status must be CONFIRMED or DECLINED"})
		return
	}
...
func (h *creditHandler) listPending(w http.ResponseWriter, r *http.Request) {
	out, err := h.credits.ListPendingForUser(r.Context(), sharedapi.CurrentUser(r).ID)
	if err != nil {
		writeError(w, r, err)
		return
	}
