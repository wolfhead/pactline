...
func completeWithCredit(t *testing.T, h http.Handler, nomineeID string, role string, evidence string) domain.Bounty {
	...
	rec := do(t, h, http.MethodPost, "/api/legacy/bounties/"+b.ID.String()+"/credits", engCID, body)
	require.Equal(t, http.StatusCreated, rec.Code)
	var c domain.Credit
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &c))
	require.Equal(t, http.StatusOK,
		do(t, h, http.MethodPost, "/api/legacy/credits/"+c.ID.String()+"/respond", nomineeID,
...
func TestWorkFeedIncludesCompletedAndAbandoned(t *testing.T) {
	...
	rec := do(t, h, http.MethodGet, "/api/legacy/works", pmID, nil)
...
func TestWorkFeedOmitsUnconfirmedCredits(t *testing.T) {
	...
	require.Equal(t, http.StatusCreated,
		do(t, h, http.MethodPost, "/api/legacy/bounties/"+b.ID.String()+"/credits", engCID,
...
func TestPortfolioListsOnlyConfirmedCredits(t *testing.T) {
	...
	require.Equal(t, http.StatusOK,
		do(t, h, http.MethodPost, "/api/legacy/credits/"+declinedCredit.ID.String()+"/respond", engDID,
	...
	rec := do(t, h, http.MethodGet, "/api/legacy/users/"+engDID+"/portfolio", pmID, nil)
	require.Equal(t, http.StatusOK, rec.Code)

	var works []legacyapi.WorkView
...
func TestPortfolioOfUnknownUserIs404(t *testing.T) {
	h := newTestServer(t)
	rec := do(t, h, http.MethodGet, "/api/legacy/users/11111111-1111-1111-1111-111111111111/portfolio", pmID, nil)
...
func TestPortfolioOfUncreditedUserIsEmpty(t *testing.T) {
	...
	rec := do(t, h, http.MethodGet, "/api/legacy/users/"+stewardID+"/portfolio", pmID, nil)
	require.Equal(t, http.StatusOK, rec.Code)
	var works []legacyapi.WorkView
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &works))
...
func findWork(feed []legacyapi.WorkView, bountyID uuid.UUID) *legacyapi.WorkView {
	for i := range feed {
