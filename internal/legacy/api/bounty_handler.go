...
type createBountyRequest struct {
	...
	// POST /api/legacy/bounties/{id}/difficulty endpoint.
...
func (h *bountyHandler) create(w http.ResponseWriter, r *http.Request) {
	...
	if !me.HasRole(userdomain.UserRoleSponsor) && !me.HasRole(userdomain.UserRoleSteward) {
	...
	if req.Title == "" {
		sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "title is required"})
		return
	}
	if req.Visibility == "" {
	...
	sharedapi.WriteJSON(w, http.StatusCreated, created)
}
...
func (h *bountyHandler) get(w http.ResponseWriter, r *http.Request) {
	...
	if b.Status == domain.StatusDraft && !canViewDraft(me, b) {
		...
		writeError(w, r, userdomain.ErrNotFound)
		return
	}
	sharedapi.WriteJSON(w, http.StatusOK, b)
}
...
func canViewDraft(u userdomain.User, b domain.Bounty) bool {
...
func (h *bountyHandler) list(w http.ResponseWriter, r *http.Request) {
	...
	f := store.BountyFilter{
		...
		Viewer:      &store.DraftViewer{ID: me.ID, IsSteward: me.HasRole(userdomain.UserRoleSteward)},
	}
	for _, s := range q["status"] {
		st := domain.Status(s)
		if !domain.IsValidStatus(st) {
			sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "status is not a known value: " + s})
			return
		}
	...
	if t := q.Get("type"); t != "" {
		...
		if !domain.IsValidBountyType(bt) {
			sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "type is not a known value: " + t})
			return
		}
		f.Type = &bt
	}
	if v := q.Get("claimed_by"); v != "" {
		id, err := uuid.Parse(v)
		if err != nil {
			sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "claimed_by is not a UUID"})
			return
		}
		f.ClaimedBy = &id
	}
	if v := q.Get("sponsor_id"); v != "" {
		id, err := uuid.Parse(v)
		if err != nil {
			sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "sponsor_id is not a UUID"})
			return
		}
		f.SponsorID = &id
	}
...
func (h *bountyHandler) transition(w http.ResponseWriter, r *http.Request) {
	...
	if !domain.IsValidStatus(req.To) {
		sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "to is not a known status: " + string(req.To)})
		return
	}
	if req.Completion != "" && !domain.IsValidCompletion(req.Completion) {
		sharedapi.WriteJSON(w, http.StatusBadRequest, sharedapi.ErrorBody{Error: "completion is not a known value: " + string(req.Completion)})
		return
	}
...
func applyTransitionFormFields(b *domain.Bounty, me userdomain.User, req transitionRequest) {
...
func authorizeTransition(me userdomain.User, b domain.Bounty, to domain.Status) error {
...
func applyTransitionEffects(b *domain.Bounty, me userdomain.User, to domain.Status) {
	now := time.Now().UTC()
	switch to {
