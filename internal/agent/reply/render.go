...
func (r Renderer) projectStatus(
	...
	fmt.Fprintf(
		...
		"**任务概览**\n\n- 全部：**%d**\n- Backlog 阶段：%d\n- Ready：%d\n- 进行中：%d\n- 验收中：%d\n- 完成：%d\n- 已取消：%d\n- 未排入 Milestone：%d\n- 逾期：%d\n- 依赖阻塞：%d",
		project.TaskCount,
		project.PhaseCounts.Backlog,
		project.PhaseCounts.Ready,
		project.PhaseCounts.InProgress,
		project.PhaseCounts.InReview,
		project.PhaseCounts.Done,
		project.PhaseCounts.Cancelled,
...
func (r Renderer) milestoneStatus(
	...
	fmt.Fprintf(
		...
		milestone.CompletionRatio*100,
		milestone.TaskCount,
		milestone.PhaseCounts.Backlog,
		milestone.PhaseCounts.Ready,
		milestone.PhaseCounts.InProgress,
		milestone.PhaseCounts.InReview,
		milestone.PhaseCounts.Done,
		milestone.PhaseCounts.Cancelled,
		milestone.OverdueCount,
		milestone.BlockedCount,
	)
	if overview.Outcome != "" {
