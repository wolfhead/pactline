...
func TestResolveProjectUsesEncodedPathAndPrivateToken(t *testing.T) {
	...
	project, err := client.ResolveProject(
		context.Background(), server.URL, "group/subgroup/repo", []byte("secret"), "request-1",
	)
	require.NoError(t, err)
	require.Equal(t, int64(17), project.ID)
	require.Equal(t, "group/subgroup/repo", project.PathWithNamespace)
	require.Equal(t, "main", project.DefaultBranch)
}

func TestGetMergeRequestReturnsConfirmedObservation(t *testing.T) {
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	mergeRequest, err := client.GetMergeRequest(
		context.Background(), server.URL, 17, 42, []byte("secret"), "request-2",
	)
	require.NoError(t, err)
	require.Equal(t, int64(91), mergeRequest.ID)
	require.Equal(t, domain.GitLabObservationConfirmed, mergeRequest.Observation.Status)
	require.Equal(t, "abc123", mergeRequest.Observation.HeadSHA)
}
...
func TestProviderErrorsAreClassified(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(fmt.Sprintf("status_%d", testCase.status), func(t *testing.T) {
			...
			_, err := client.ResolveProject(
				context.Background(), server.URL, "group/repo", []byte("secret"), "request-3",
			)
			require.Error(t, err)
...
func TestClientRejectsOversizedAndInvalidJSONResponses(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, err := client.ResolveProject(
				context.Background(), server.URL, "group/repo", []byte("secret"), "request-4",
			)
			require.Error(t, err)
...
func TestClientRejectsCrossOriginRedirect(t *testing.T) {
	...
	_, err := client.ResolveProject(
		context.Background(), source.URL, "group/repo", []byte("secret"), "request-5",
	)
	require.Error(t, err)
...
func TestClientHonorsTimeout(t *testing.T) {
	...
	_, err := client.ResolveProject(
		context.Background(), server.URL, "group/repo", []byte("secret"), "request-6",
	)
	require.Error(t, err)
	require.Equal(t, ErrorUnreachable, ErrorCategoryOf(err))
}
