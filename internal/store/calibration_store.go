...
const calibrationColumns = `id, bounty_id, quarter, original_value, calibrated_value, calibrated_score, note, created_by, created_at`
...
func (s *CalibrationStore) Create(ctx context.Context, c domain.Calibration) (domain.Calibration, error) {
	...
	row := s.db.Pool.QueryRow(ctx, `
		INSERT INTO calibrations (id, bounty_id, quarter, original_value, calibrated_value, calibrated_score, note, created_by)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
		RETURNING `+calibrationColumns,
		c.ID, c.BountyID, c.Quarter, c.OriginalValue, c.CalibratedValue, c.CalibratedScore, c.Note, c.CreatedBy)
	...
	slog.Info("calibration recorded",
		...
		"original_value", out.OriginalValue, "calibrated_value", out.CalibratedValue,
		"calibrated_score", out.CalibratedScore, "created_by", out.CreatedBy)
	return out, nil
}
...
func scanCalibration(s scanner) (domain.Calibration, error) {
	...
	if err := s.Scan(&c.ID, &c.BountyID, &c.Quarter, &c.OriginalValue, &c.CalibratedValue,
